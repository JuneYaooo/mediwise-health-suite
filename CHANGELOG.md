...
- Added a bilingual case walkthrough to the Chinese and English README for local record keeping, written as a short conversation rather than a command reference: a seven-page fictional teaching case (two admissions, lab reports, imaging, and a discharge record) taken through what the user says in plain language, the confirmation step before anything is saved, the structured records, the health card, and four follow-up answers — with the seven source images as thumbnails under `docs/images/case-0N-*.jpg` and the resulting card as `docs/images/case-health-card.png`. No product code changed.
...
### Changed
- README cleanup after the walkthrough landed: the walkthrough is now a subsection of the "upload medical records" scenario rather than its own top-level section, so the flow is described once and neither navigation bar carries a separate case-walkthrough entry pointing into the middle of that scenario. The English README gained the matching "Three common scenarios" chapter (meals from a photo, medical records, device data) that the Chinese README already had, so both now place the walkthrough in the same spot and the two files read in the same order. The synthetic demo card images (personal, English personal, and family) were dropped from both READMEs and deleted from `docs/images/`, so the teaching case is the only worked example shown and no unreferenced card render is left behind. The version footer and the Requirements / 运行要求 block were dropped from both READMEs; the environment requirements remain in `SKILL.md` and `docs/INSTALL_AGENT.md`. Documentation only.
