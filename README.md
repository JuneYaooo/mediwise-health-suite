...
项目及整套能力统一称为 **MediWise Health Suite**，下文简称 **MediWise**。`mediwise-health-tracker`、`diet-tracker` 等名称只是仓库内部的 Skill 模块 ID，不是多个产品名；图片汇总能力统一称为“健康记录卡片”，家庭概览版本称为“家庭健康记录卡片”。
...
当你说“帮我生成最近 7 天的健康记录卡片”，MediWise 会根据本地档案生成一张可以直接在聊天中查看的图片。个人版把指标趋势、饮食摄入、运动消耗、步数和睡眠压缩到同一张卡片，再用个人健康时间轴按日期串联指标更新、日常记录、近期就医、检验和检查；家庭版则突出需要关注的成员和全家的近期医疗时间线。

#### 个人版

<p align="center">
  <img src="docs/images/health-card-example.png" width="720" alt="MediWise 个人健康记录卡片示例，包含紧凑指标趋势、摄入与消耗、睡眠、个人健康时间轴和在用药">
</p>

个人时间轴最多展示 10 条近期动态，严格按日期倒序排列；同一天内医疗事件优先，同时纳入最近指标、饮食、运动和睡眠记录。卡片不是固定模板：明确异常和到期提醒优先，其次根据各模块的实际记录量动态调整顺序，并放大重点指标或记录最丰富的生活方式面板；用户明确询问某一部分时，则以该部分为主。个人版中的热量和营养素按“有饮食记录的日期”计算日均值，不会把未记录日当作零摄入；运动消耗只代表已记录的活动，不会在缺少完整基础代谢与日常活动数据时虚构“热量缺口”。检验结果也只统计报告中明确带有高、低或异常标记的项目。

#### 家庭版

<p align="center">
  <img src="docs/images/family-health-card-example.png" width="720" alt="MediWise 家庭健康记录卡片示例，包含姓名与身份、关注状态、近期数据覆盖和家庭医疗时间线">
</p>

家庭版面向“一个人管理本人和多位家人”的本地场景。每位成员始终显示为“姓名（身份）”；有告警、明确异常或到期提醒的成员自动排在前面，其他成员再按近期数据覆盖度排序。卡片只做紧凑概览；需要查看营养明细、完整趋势或在用药时，再生成对应成员的个人版。

> 两张示例均由项目内置的真实卡片生成链路制作，使用的是虚构成员和虚构健康数据，仅用于展示效果，不代表医疗建议。趋势图使用内联 SVG，生成时不依赖外部图表服务。
...
家庭版不要求指定单个姓名。没有记录的项目会明确显示为暂无数据，卡片不会为了补全版面编造指标、营养数据或检验结论。默认范围为最近 7 天，也可以改成最近 30 天等时间范围。
...
把仓库地址发给 OpenClaw、Codex、Claude Code、Cursor、Trae 或其他具备终端和网络访问能力的 AI 助手即可：
...
仓库已经包含给安装 Agent 阅读的完整执行说明。AI 会自己检查环境、选择安装目录、安装依赖、配置个人本地模式和图片/PDF 识别，并运行验收脚本；你不需要在提示词里重复这些实现细节，也不需要手动执行命令。
...
推荐在安装时一起启用图片/PDF 识别。可以使用能直接读取图片的多模态模型，也可以使用普通文本模型搭配 OCR；内置的本地 OCR 路径是 PaddleOCR。只有脱敏测试图片、扫描 PDF 和结构化解析都实际通过后，才算配置完成。如果安装时跳过了，之后直接对具备本机配置权限的 AI 助手说：

```text
请帮我启用 MediWise 的图片和 PDF 识别，并用一份脱敏文件测试。能读图的模型直接配置多模态识别；不能读图的模型请搭配 OCR。
```

PaddleOCR 可以在本地提取普通图片和扫描 PDF 中的中文文字，再由文本模型整理成结构化记录；多模态模型更适合复杂版面、表格和图表。AI 应优先使用系统安全凭据存储或受保护的本地输入方式，不在聊天中收集 Key。云端视觉或 OCR 服务会收到完整图片或 PDF 页面；报告可能包含姓名、证件号、病历号等个人信息，敏感材料请先脱敏，或使用本地能力。
...
模型上下线和输入能力可能变化。配置 Agent 必须读取所选服务商当前模型信息，并用脱敏图片和临时扫描 PDF 验证识别及结构化解析，不能只凭模型名称判断“已经支持视觉”。
