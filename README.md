...
[一张图看懂](#一张图看懂-mediwise) · [常用场景](#三个常用场景) · [健康卡片与目标](#健康卡片与目标) · [5 分钟上手](#5-分钟上手) · [功能全览](#功能全览) · [隐私与安全](#隐私与安全) · [文档中心](docs/README.md)
...
<table>
  <tr>
    <td width="50%" align="center"><img src="docs/images/health-card-example.png" alt="MediWise 个人健康卡片示例"></td>
    <td width="50%" align="center"><img src="docs/images/family-health-card-example.png" alt="MediWise 家庭健康卡片示例"></td>
  </tr>
  <tr>
    <td align="center">个人健康卡片</td>
    <td align="center">家庭健康卡片</td>
  </tr>
</table>
...
普通健康数据生成静态“健康卡片”。八域观察引擎覆盖体重、睡眠、生命体征、摄入、活动、服药记录、记录行为和家庭记录；每个域只定义读取哪类记录、怎样折叠同日数据以及如何陈述数字方向。卡片只复述实际记录、空白和同期变化，不给出医学判断，也不把相关性写成原因。

体重健康卡片是这套引擎的展示案例，而不是产品边界。它会先把同日多次测量折叠为中位数，用 Theil–Sen 稳健趋势区分“今天的波动”和“一段时间的方向”，再把同期已有的摄入、活动和睡眠记录放在一起观察。

<p align="center">
  <img src="docs/images/weight-story-template-gallery-24.png" width="920" alt="MediWise 体重健康卡片的 24 套模板">
</p>
...
- **静态卡片**：单域可生成自包含 HTML、SVG 和 1080×1440 PNG；普通健康数据不生成视频。
- **用户确认的目标**：用户明确设定行动目标后，MediWise 才启用打卡和周期/累计进度；只有出现节奏、恢复、连续稳定、半程或完成等新行为含义时才生成静态目标证据卡。普通步数、体重或心率不会静默创建目标或自动打卡。

```text
帮我观察最近 30 天的睡眠记录，生成一张默认脱敏的健康卡片。
把最近 14 天有记录的健康维度做成一个 MP4，每个镜头也单独保存成图片。
生成一张体重健康卡片，把同期已有的摄入、活动和睡眠记录放在一起。
为家庭记录生成健康卡片，不显示姓名，也不比较成员。

帮我设计一个“每周运动 3 次、连续 4 周”的目标，先让我确认，再开始打卡。
```
...
| 生成近期健康卡片 | 生成个人或家庭的近期健康概览图片 |
| 生成单域健康卡片 | 用 24 套模板观察体重、睡眠、生命体征、摄入、活动、服药记录、记录行为或家庭记录，导出默认脱敏的 HTML、SVG 或 PNG；体重是展示案例，旧体重真相卡继续兼容 |
| 管理健康目标 | 用户确认后创建行动目标，记录主动打卡或明确的运动来源，计算进度，并在出现新行为含义时生成静态目标证据卡 |
...
- 健康卡片默认隐藏姓名、精确日期和域内敏感值；体重展示案例还默认隐藏绝对体重与目标体重，家庭记录域不读取成员姓名或其他身份信息。目标证据卡默认隐藏姓名和精确日期，但会展示用户设定的目标标题与触发当时的冻结证据。开启个人字段后，分享前请再次检查产物内容。
...
运行要求：Python 3.8+、Node.js 18+、SQLite 3.x，以及一个能够加载 Skills、访问本地文件并执行脚本的 Agent；支持 Linux、macOS 和 Windows。生成本地 PNG 健康卡片、目标证据卡、旧体重真相卡或 PDF 还需要 Chrome/Chromium；HTML、SVG、纯文字记录和查询不需要这些渲染工具。使用 OpenClaw 时要求 2026.3.0+。
