...
[一张图看懂](#一张图看懂-mediwise) · [常用场景](#三个常用场景) · [体重译报](#体重译报) · [5 分钟上手](#5-分钟上手) · [功能全览](#功能全览) · [隐私与安全](#隐私与安全) · [文档中心](docs/README.md)
...
**把秤上的数字，翻译成一段看得懂、愿意保存的身体近况。**

体重是结果信号，不是全部。体重译报会先把同日多次测量折叠为中位数，用 Theil–Sen 稳健趋势区分“今天的波动”和“一段时间的方向”，再把同期已有的饮食摄入、运动和睡眠记录放在一起分析。每张卡都有一段完整解读，但只描述同期变化和数据覆盖，不把相关性写成原因。
...
- **四域一起看**：体重看稳健趋势；摄入只统计有记录日；运动时长与已记录消耗不冒充全天消耗；睡眠只描述记录时长。
- **24 张不是换皮**：每套都有唯一视觉轮廓和主导任务，分别以体重、摄入、运动、睡眠、记录行为或综合解读为主角。
- **先总结“你最近怎么了”**：根据真实记录生成「后半场换挡」「今天抢镜，长线没改剧本」「四线同框，剧情待续」等阶段肖像，再给出可核对证据；数据不足时只说「线索还在加载」。
- **按自媒体阅读顺序设计**：结论先行、数字证明、完整分析、保存理由，既能在信息流里停住视线，也不靠标题党制造健康焦虑。
- **专业但不无聊**：同一组事实可被写成气象简报、船长日志、编辑导语、唱片内页或档案结论；风格由场景、偏好、可用数据和防重复概率共同决定。
- **默认可分享**：隐藏姓名、绝对体重、目标体重和精确日期，可生成自包含 HTML 和 1080×1440 PNG。

```text
帮我分析最近 30 天的体重、饮食、运动和睡眠记录。
生成一张默认脱敏的体重译报。
这次用轻松一点、不要和最近重复的风格。
```
...
| 生成体重译报 | 用 24 套动态模板生成默认脱敏、适合保存或分享的体重趋势 HTML/PNG 图卡；旧体重真相卡继续兼容 |
...
- 体重译报和旧体重真相卡都默认隐藏姓名、绝对体重、目标体重、精确日期、用药和检验数据；开启个人字段后，分享前请再次检查图片内容。
...
运行要求：Python 3.8+、Node.js 18+、SQLite 3.x，以及一个能够加载 Skills、访问本地文件并执行脚本的 Agent；支持 Linux、macOS 和 Windows。生成本地 PNG 健康记录卡片、体重译报、旧体重真相卡或 PDF 还需要 Chrome/Chromium；HTML 和纯文字记录、查询不需要。使用 OpenClaw 时要求 2026.3.0+。
