...
个人卡片会整理已有的指标、饮食运动、睡眠、医疗记录和用药。家庭卡片只展示每位成员的当前状态、在用药、服药或复查提醒，以及需要注意的事项，不展示时间轴。示例使用虚构人物和虚构数据，不代表医疗建议。
...
请同时配置图片和 PDF 识别，完成后帮我检查是否可以正常使用。
```
...
- 只有你主动配置云端图片识别或其他在线服务时，相关内容才会发送给对应服务商。
...
本项目仅用于健康信息记录、整理和趋势参考，不构成医疗诊断、治疗方案或紧急医疗服务。
