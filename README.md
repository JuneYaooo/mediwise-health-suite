...
个人时间轴最多展示 10 条近期动态，严格按日期倒序排列；同一天内医疗事件优先，同时纳入最近指标、饮食、运动和睡眠记录。个人版中的热量和营养素按“有饮食记录的日期”计算日均值，不会把未记录日当作零摄入；运动消耗只代表已记录的活动，不会在缺少完整基础代谢与日常活动数据时虚构“热量缺口”。检验结果也只统计报告中明确带有高、低或异常标记的项目。
...
家庭版面向“一个人管理本人和多位家人”的本地场景。每位成员始终显示为“姓名（身份）”，卡片只做紧凑概览；需要查看营养明细、完整趋势或在用药时，再生成对应成员的个人版。
