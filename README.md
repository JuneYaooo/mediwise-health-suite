...
- **单域卡与多维视频**：单域可生成自包含 HTML、动画 SVG 和 1080×1440 冻结 PNG；个人综合译报把当前所有有有效记录的维度编成 1080×1920 MP4，并同时保存封面、总览、每域分析卡和边界页的独立 PNG。
...
| 生成健康译报 | 单域用 24 套模板观察体重、睡眠、生命体征、摄入、活动、服药记录、记录行为或家庭记录，导出默认脱敏的 HTML、动画 SVG 或冻结 PNG；个人综合译报可把所有有记录域导出为 MP4 和逐镜头 PNG；体重是展示案例，旧体重真相卡继续兼容 |
...
运行要求：Python 3.8+、Node.js 18+、SQLite 3.x，以及一个能够加载 Skills、访问本地文件并执行脚本的 Agent；支持 Linux、macOS 和 Windows。生成本地 PNG 健康记录卡片、健康译报冻结 PNG、综合译报分镜、旧体重真相卡或 PDF 还需要 Chrome/Chromium；把分镜编码成 MP4 还需要 FFmpeg/ffprobe。健康译报的 HTML、动画 SVG，以及纯文字记录和查询不需要这些渲染工具。使用 OpenClaw 时要求 2026.3.0+。
