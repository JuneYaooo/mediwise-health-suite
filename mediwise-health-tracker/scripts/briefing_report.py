...
def _sparkline(metric_type: str, points: list[dict], locale: str) -> str:
    ...
    if metric_type == "blood_pressure":
        content = path([p.get("systolic") for p in points], "#D66548") + path([p.get("diastolic") for p in points], "#246BCE")
    else:
        content = path([_metric_number(p, metric_type) for p in points], "#246BCE")
    label = _escape(METRIC_NAMES[locale].get(metric_type, metric_type))
    return f'<svg class="spark" viewBox="0 0 {width} {height}" role="img" aria-label="{label}"><line x1="{pad}" y1="{height-pad}" x2="{width-pad}" y2="{height-pad}" stroke="#DCE6F0"/>{content}</svg>'
...
*{{box-sizing:border-box}} body{{margin:0;background:var(--page);color:var(--ink);font:14.5px/1.62 "Avenir Next","Segoe UI Variable","Segoe UI","PingFang SC","Microsoft YaHei",sans-serif;line-break:strict;overflow-wrap:break-word;font-synthesis:none}} .container{{max-width:1040px;margin:auto;padding:26px}}
...
.brand{{display:flex;align-items:center;gap:14px}} .mark{{display:grid;place-items:center;width:44px;height:44px;border-radius:13px;background:#EAF4FF;color:#175D9B;font-weight:800;font-size:22px;box-shadow:0 8px 18px rgba(4,37,68,.16)}} h1{{font-size:29px;line-height:1.22;margin:0;letter-spacing:-.25px;text-wrap:balance}} .subtitle{{margin-top:16px;color:#D9EAFE;font-size:14px}} .privacy{{display:inline-block;margin-top:14px;padding:5px 11px;border:1px solid rgba(255,255,255,.28);border-radius:99px;color:#EFF7FF;font-size:12px;background:rgba(4,35,63,.10)}}
.summary-strip{{display:grid;grid-template-columns:repeat(3,1fr);background:var(--surface);border:1px solid var(--line);border-radius:18px;margin:18px 0;padding:17px;box-shadow:0 9px 28px rgba(21,63,104,.07)}} .summary-strip>div{{padding:2px 22px;border-right:1px solid #E2EAF2}} .summary-strip>div:last-child{{border:0}} .summary-strip b{{font-size:26px;display:block;line-height:1.1;font-variant-numeric:tabular-nums}} .summary-strip span{{font-size:12px;color:var(--muted)}} .green{{color:var(--cyan)}} .blue{{color:var(--primary)}} .coral{{color:var(--coral)}} .gold{{color:var(--gold)}}
section{{background:var(--surface);border:1px solid var(--line);border-radius:19px;padding:22px;margin:14px 0;box-shadow:0 10px 30px rgba(21,63,104,.065)}} .section-featured{{border-color:#A8C9E8;box-shadow:0 12px 34px rgba(31,91,147,.11)}} .section-title{{display:flex;align-items:center;gap:10px;margin-bottom:14px}} .section-title>span{{font-size:10px;font-weight:800;color:#2F6EA5;border:1px solid #C5D9EC;background:#F2F7FC;border-radius:99px;padding:2px 7px}} h2{{font-size:19px;line-height:1.3;margin:0;color:var(--ink-strong);text-wrap:balance}} h3{{margin:0;font-size:15px;color:var(--ink-strong)}} p{{text-wrap:pretty}} .muted{{color:var(--muted);font-size:11px}} .empty{{padding:20px;text-align:center;border:1px dashed #B9CDE0;border-radius:12px;background:#F8FAFD;color:var(--muted)}} .empty.compact{{padding:14px 9px;margin-top:8px}}
.metric-grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}} .metric-grid.focused{{grid-template-columns:repeat(5,1fr)}} .metric-card{{padding:14px;border-radius:13px;background:var(--surface-muted);border:1px solid #D9E4EF;min-width:0}} .metric-card.featured{{grid-column:span 2;background:#F1F7FD;border-color:#B8D0E7}} .metric-card.featured .spark{{height:50px}} .metric-head,.meta{{display:flex;justify-content:space-between;gap:6px}} .metric-head b{{font-size:12px;color:var(--ink-strong)}} .metric-head span{{font-size:9px;background:var(--primary-soft);color:var(--primary);border-radius:99px;padding:2px 6px;white-space:nowrap}} .metric-value{{font-size:25px;font-weight:760;margin-top:7px;color:var(--ink-strong);font-variant-numeric:tabular-nums}} .metric-value small,.big small{{font-size:10px;color:var(--muted)}} .spark{{display:block;width:100%;height:42px;margin:6px 0}} .meta{{font-size:9px;color:#587087}} .metric-title{{margin-top:16px}}
.wellness-grid{{display:grid;grid-template-columns:1.05fr 1fr 1fr;gap:10px}} .wellness-grid.focused{{grid-template-columns:repeat(4,minmax(0,1fr))}} .featured-panel{{grid-column:span 2}} .panel{{min-height:160px;border-radius:14px;padding:15px;border:1px solid #D9E4EF;background:#F8FAFD}} .intake{{background:#F2F7FE;border-color:#D4E2F2}} .activity{{background:#F1F8FC;border-color:#D2E7F0}} .sleep{{background:#FCF6F3;border-color:#EFDCD3}} .eyebrow{{font-size:11px;font-weight:760;color:#496780}} .big{{font-size:25px;font-weight:780;margin-top:7px;font-variant-numeric:tabular-nums}} .macro,.sleep-row{{display:grid;grid-template-columns:repeat(2,1fr);gap:5px;margin-top:9px}} .macro span,.sleep-row span{{font-size:9px;color:#526B82}} .macro b,.sleep-row b{{display:block;font-size:11px;color:#24445F}} .step-box{{margin-top:9px;padding-top:8px;border-top:1px solid #D2E3EE;display:grid;grid-template-columns:1fr auto}} .step-box b{{font-size:16px;color:var(--cyan)}} .step-box small{{grid-column:1/3;color:#587087}} .top-gap{{margin-top:10px}} .scope-note{{margin:10px 2px 0;font-size:9px;color:#587087}}
.table-wrap{{overflow:hidden;border:1px solid var(--line);border-radius:13px}} table{{width:100%;border-collapse:collapse;font-variant-numeric:tabular-nums}} th{{background:#EDF4FA;color:#274E70;font-size:11px;text-align:left}} th,td{{padding:10px 12px;border-bottom:1px solid #E8EEF4}} tr:last-child td{{border:0}} td{{font-size:11px}}
...
.family-grid{{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}} .family-card{{padding:18px;border:1px solid #D6E2ED;border-radius:16px;background:#FBFCFE}} .family-card.featured-member{{border-color:#E7A48E;background:#FFF9F7;box-shadow:0 10px 24px rgba(174,78,49,.10)}} .family-card-head{{display:flex;justify-content:space-between;gap:10px;margin-bottom:12px}} .family-card-head p{{margin:3px 0;color:var(--muted);font-size:10px}} .family-grid>.family-card:nth-child(odd):last-child{{grid-column:1/-1;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));column-gap:14px}} .family-grid>.family-card:nth-child(odd):last-child .family-card-head{{grid-column:1/-1}} .status{{height:max-content;padding:4px 8px;border-radius:99px;background:#EAF3FB;color:#195F93;font-size:9px;white-space:nowrap}} .status.watch{{background:#FCEBE5;color:#A64C36}} .family-block{{padding:11px 0;border-top:1px solid #E3EBF2}} .family-block-title{{font-size:10px;font-weight:760;color:#46657E;margin-bottom:7px}} .family-metrics{{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}} .family-metrics span{{padding:8px;background:#EFF5FA;border-radius:9px;min-width:0}} .family-metrics small,.family-metrics b{{display:block}} .family-metrics small{{font-size:9px;color:var(--muted)}} .family-metrics b{{font-size:13px;margin-top:2px;white-space:nowrap;color:var(--ink-strong)}} .family-metrics i{{font-size:8px;font-style:normal;color:#587087}} .family-state-note,.family-empty{{padding:8px 10px;border-radius:9px;background:#F1F5F9;color:var(--muted);font-size:10px}} .family-empty.clear-state{{background:#EAF3FB;color:#195F93}} .family-list{{display:grid;gap:6px}} .family-list-item{{display:flex;align-items:center;gap:8px;padding:8px 9px;border-radius:9px;background:#F1F5F9;min-width:0}} .family-list-item.medication{{justify-content:space-between;background:#EDF4FC}} .family-list-item.medication div{{min-width:0}} .family-list-item.medication b,.family-list-item.medication small{{display:block}} .family-list-item.medication b{{font-size:11px;color:#234D70}} .family-list-item.medication small{{font-size:9px;color:#526B82;margin-top:1px}} .family-list-item.medication em{{font-size:8px;font-style:normal;color:var(--primary);background:#fff;padding:3px 6px;border-radius:99px;white-space:nowrap}} .family-list-item.attention{{align-items:flex-start}} .family-list-item.attention i{{width:6px;height:6px;border-radius:50%;background:var(--primary);margin-top:6px;flex:none}} .family-list-item.attention.warning i{{background:#D49A30}} .family-list-item.attention.alert i{{background:#D65F45}} .family-list-item.attention p{{margin:0;font-size:10px;color:#405E77}} .family-more{{font-size:9px;color:var(--muted);padding-left:3px}}
.timeline{{padding-left:6px}} .timeline-item{{display:grid;grid-template-columns:78px 12px 1fr;gap:9px;min-height:55px}} .timeline-item time{{font-size:10px;color:var(--muted);padding-top:2px;font-variant-numeric:tabular-nums}} .timeline-item>span{{position:relative}} .timeline-item>span:before{{content:"";position:absolute;width:7px;height:7px;border-radius:50%;background:var(--coral);top:5px;left:2px}} .timeline-item>span:after{{content:"";position:absolute;width:1px;background:#D9E4EF;top:15px;bottom:0;left:5px}} .timeline-item:last-child>span:after{{display:none}} .timeline-item b{{font-size:12px;color:var(--ink-strong)}} .timeline-item small{{margin-left:7px;padding:2px 6px;border-radius:99px;background:#F4E9E5;color:#9B5A47;font-size:8px}} .timeline-item p{{font-size:11px;color:#536D83;margin:1px 0}} .personal-timeline{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));column-gap:25px}} .personal-timeline .timeline-item{{grid-template-columns:72px 12px 1fr;min-height:52px}} .personal-timeline .metric-event>span:before{{background:var(--primary)}} .personal-timeline .metric-event small{{background:var(--primary-soft);color:var(--primary)}} .personal-timeline .food-event>span:before{{background:#557FBA}} .personal-timeline .food-event small{{background:#EDF3FA;color:#446A9E}} .personal-timeline .activity-event>span:before{{background:var(--cyan)}} .personal-timeline .activity-event small{{background:var(--cyan-soft);color:#126985}} .personal-timeline .sleep-event>span:before{{background:#C8775E}} .personal-timeline .sleep-event small{{background:#F7EAE5;color:#A45C47}}
.footer{{text-align:center;color:#526B82;font-size:10px;padding:15px 10px 6px}} .footer b{{display:block;color:#2B5475;margin:3px}} @media(max-width:700px){{.container{{padding:12px}}.header{{padding:25px}}.metric-grid,.metric-grid.focused{{grid-template-columns:repeat(2,1fr)}}.wellness-grid,.wellness-grid.focused,.family-grid,.personal-timeline{{grid-template-columns:1fr}}.family-grid>.family-card:nth-child(odd):last-child{{grid-column:auto;display:block}}.featured-panel{{grid-column:auto}}.attention-list{{grid-template-columns:1fr}}.compact-attention{{align-items:flex-start;flex-direction:column}}.summary-strip>div{{padding:2px 8px}}.family-metrics{{grid-template-columns:repeat(2,1fr)}}}} @media print{{body{{background:white}}.container{{max-width:none}}section,.header{{box-shadow:none;break-inside:avoid}}}}
