...
def _situation_portrait(weight: Mapping[str, object], intake: dict, activity: dict, sleep: dict, days: int) -> dict:
    ...
    # Every branch below writes weight-specific prose — 「体重、摄入、运动和睡眠」 in the
    # `four-signals` hook, 体重 again through `_weight_sentence` in `summary`.  That is
    # correct here and only here: this module is reachable only from
    # `analyze_weight_management`, which only `weight_truth_card`'s weight branch calls.
    # Nothing in this file may be generalised by parameterising the copy — the axis lives
    # in `adapters.companions_for`, and both consumers gate on it (`render._management_details`
    # drops the whole block, `selector.detect_story_moments` drops the companion moments).
    # A second domain that wants a synthesis needs its own companion declaration and its
    # own portrait, not this one with a subject substituted in.
