---
name: weight-manager
description: "Weight management: record user-provided weight goals, track progress, translate daily fluctuations into robust trends, generate personalized share-safe health cards across 24 styles, preserve legacy weight truth cards, log exercise, calculate BMI/BMR/TDEE, and analyze body composition. Integrates with health-goals, diet-tracker, and mediwise-health-tracker."
---
...
| source | 记录来源，默认 manual |
| source_record_id | 外部来源记录 ID（如有） |
| note | 备注 |
...
| select-health-card-style / select-weight-card-style | select-style | --member-id | --domain（weight/sleep/vitals/intake/activity/adherence/records/family，默认 weight）、--days、--scene、--tone、--density、--preferred-style、--disliked-style、--recent-style、--pinned-style、--surprise-level、--seed | 为健康卡片从共享模板中进行可解释、非均匀、可复现的选择；weight 动作名仅作兼容 |
...
| generate-domain-health-card / generate-weight-story-card | generate-story | --member-id | --domain（weight/sleep/vitals/intake/activity/adherence/records/family，默认 weight）、--days、--scene、--tone、--density、--style（默认 auto）、--format（html/png/svg/both/all）、隐私开关、偏好/历史、--seed、--no-save-history | 生成「MediWise 健康卡片」：观察所选域的真实记录，选择共享模板之一，导出 HTML/PNG/SVG 并记录最近风格；story 动作名仅作兼容 |
...
分析会先把同一天的多次体重测量聚合为中位数，再使用 Theil–Sen 稳健斜率估计长期方向。少于 7 个记录日或覆盖跨度不足 7 天时，只显示“记录不足”，不宣称趋势已经形成。体重健康卡片还会并列分析同期已有的饮食摄入、运动和睡眠记录，生成一段综合解读，并依据真实模式形成「阶段肖像」和默认脱敏的分享包装。分享结构采用结论先行、数字证明、完整分析、保存理由，但必须返回 `clickbait: false`。饮食只统计有营养数据的记录日，未记录日不按 0 kcal；运动消耗不冒充全天总能量消耗，也不与摄入相减生成热量缺口；睡眠只描述记录时长。同期变化只作为线索，不得写成体重变化的原因。文案由确定性规则产生，不调用模型自由发挥，也不预测达标日期。
...
对外统一称为 **「MediWise 健康卡片」**；当 `domain=weight` 时称为 **「MediWise 体重健康卡片」展示案例**。优先使用 `generate-domain-health-card` 与 `select-health-card-style`；`generate-weight-story-card` 与 `select-weight-card-style` 只为兼容旧 action ID，不传 `--domain` 时仍默认 `weight`。旧 `weight-truth` 和 `generate-weight-card` 是纯体重兼容入口，必须保持原行为。

### exercise.py — 运动记录

| 动作 | 子命令 | 必要参数 | 可选参数 | 说明 |
|------|--------|----------|----------|------|
| add-exercise | add | --member-id, --exercise-type | --exercise-name, --duration, --distance-meters, --calories-burned, --exercise-date, --exercise-time, --intensity, --source, --source-record-id, --note | 添加运动记录；提交成功后可关联进行中的活动目标 |
...
- 预测功能基于近期体重变化趋势，仅供参考
- 健康卡片聚焦所选域的观察展示；其中体重展示案例和旧「体重真相卡」聚焦体重波动
- 所有健康卡片都只陈述记录事实和统计趋势；相关线索不代表因果，不得据此自动生成饮食、热量或运动处方
