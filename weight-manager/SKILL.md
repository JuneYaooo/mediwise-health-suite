---
name: weight-manager
description: "Weight management: record user-provided weight goals, track progress, translate daily fluctuations into robust trends, generate a share-safe weight truth card, log exercise, calculate BMI/BMR/TDEE, and analyze body composition. Integrates with diet-tracker and mediwise-health-tracker."
---
...
对用户统一称为 **「MediWise 体重真相卡」**，是本 skill 唯一的卡片产物。

分析会先把同一天的多次体重测量聚合为中位数，再使用 Theil–Sen 稳健斜率估计长期方向，并给出最新一次称重与稳健趋势的偏差。少于 7 个记录日或覆盖跨度不足 7 天时，只显示“记录不足”，不宣称趋势已经形成。文案由确定性规则产生，不调用模型自由发挥，也不预测达标日期；相关线索不代表因果，不得写成体重变化的原因。
...
15. 需要可保存、可分享的图片时使用 `generate-weight-card` 生成体重真相卡；除非用户明确要求，否则保留默认脱敏设置
