...
// Repository root, so `shared.card_capture` — the settled-frame PNG renderer the
// card shares with the tracker's screenshot helper — is importable.
...
export async function execute(inputs, context) {
  ...
  try {
    ...
    if (ownerId) {
      ...
      log('[weight-manager] single-user mode enabled');
    }

    log(`[weight-manager] script=${script}`);
    ...
    log(`[weight-manager] script failed exit=${exitCode}`);
