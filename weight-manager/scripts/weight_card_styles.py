...
STYLE_CATALOG: Tuple[WeightCardStyle, ...] = (
    ...
    _style("direction-course", "direction", "航行记录", "身体航向", "A", "把单日波动看作浪、长期趋势看作航向", ("calm", "editorial"), ("concise", "detailed"), ("daily", "weekly", "share"), 2, False, 1.28, "common", "最新秤面与稳健趋势之间的距离", "production"),
    ...
    _style("ticket-journey", "journey", "里程成就", "健康车票", "A", "把阶段记录做成一张从此刻出发的车票", ("playful", "bold"), ("concise",), ("daily", "milestone", "share"), 1, False, 1.06, "common", "起点是第一次记录，目的地不是某个体重数字"),
    _style("passport-stamps", "journey", "里程成就", "身体护照", "B", "一次阶段观察就是一枚不评价胖瘦的印章", ("playful", "editorial"), ("detailed",), ("milestone", "share"), 7, False, 0.72, "uncommon", "只奖励观察行为与时间跨度的收藏印章"),
...
STYLES_BY_ID: Dict[str, WeightCardStyle] = {style.id: style for style in STYLE_CATALOG}


def validate_catalog() -> None:
    if len(STYLE_CATALOG) != 24:
        raise ValueError("weight-card catalog must contain exactly 24 styles")
    if len(STYLES_BY_ID) != len(STYLE_CATALOG):
        raise ValueError("weight-card style ids must be unique")
    if len(set(STYLE_LAYOUT_MODES.values())) != 24:
        raise ValueError("each weight-card style must have a unique layout mode")
    ...
    if len(family_counts) != 12 or any(count != 2 for count in family_counts.values()):
        raise ValueError("catalog must be 12 two-variant families")
