"""Dynamic renderers for the MediWise 体重译报 card system.

Professional analysis is supplied by ``weight_truth_card``.  This module only
changes narrative form and visual structure.  All 24 styles use the same
...
CARD_WIDTH = 1080
CARD_HEIGHT = 1440
PRODUCT_NAME = "MediWise 体重译报"
DISCLAIMER = "相关线索不代表因果；本卡不提供诊断或减重处方。"


CORE_STATE_COPY = {
    "insufficient": ("记录还在序章", "当前记录不足以判断长期方向"),
    "daily_up_trend_down": ("今天上浮，长期方向未改变", "单日变化与稳健长期趋势方向不同"),
    "daily_down_trend_up": ("今天回落，长期方向未改变", "单日变化与稳健长期趋势方向不同"),
    "sustained_down": ("短期与长期方向一致", "当前记录显示两个时间尺度方向相同"),
    "sustained_up": ("短期与长期方向一致", "当前记录显示两个时间尺度方向相同"),
    "daily_up_stable": ("今天上浮，长期仍近似稳定", "一天的变化尚未改写稳健趋势"),
    "daily_down_stable": ("今天回落，长期仍近似稳定", "一天的变化尚未改写稳健趋势"),
...
FAMILY_HEADLINES = {
    "weather": {
        ...
        "daily_up_trend_down": "短时有云，长期气流未改",
        "daily_down_trend_up": "短时转晴，长期气流未改",
        "stable": "今日微风，长期平稳",
    },
    "direction": {
        "insufficient": "先把点连起来，再谈航向",
        "daily_up_trend_down": "今天有浪，航向没变",
        "daily_down_trend_up": "今天退潮，航向还没掉头",
        "stable": "海面有动，航向近乎水平",
    },
    "terrain": {
        "insufficient": "地形仍在显影",
        "daily_up_trend_down": "眼前是坡，整段路仍向原方向延伸",
        "daily_down_trend_up": "眼前是谷，整段路仍向原方向延伸",
        "stable": "这是一段缓坡，不是一条直线",
    },
    "editorial": {
        "insufficient": "本期头条：暂不下结论",
        "daily_up_trend_down": "一日上浮，并未改写长期趋势",
        "daily_down_trend_up": "一日回落，并未改写长期趋势",
        "stable": "变化存在，方向近似稳定",
    },
    "capsule": {
        "insufficient": "先封存今天，不急着解释",
        "daily_up_trend_down": "把今天的浪，留给更长的时间回答",
        "daily_down_trend_up": "把今天的退潮，留给更长的时间回答",
        "stable": "封存一段平静但并不静止的日子",
    },
    "film": {
        "insufficient": "第一格胶片，已经装进来了",
        "daily_up_trend_down": "这一格上浮，不代表整卷胶片",
        "daily_down_trend_up": "这一格回落，不代表整卷胶片",
        "stable": "每一格不同，整卷仍有自己的节奏",
    },
    "rhythm": {
        "insufficient": "节拍刚刚开始",
        "daily_up_trend_down": "今天换了拍，整段节奏未变",
        "daily_down_trend_up": "今天轻了一拍，整段节奏未变",
        "stable": "变化有节拍，方向近似稳定",
    },
    "journey": {
        "insufficient": "这是一张启程票",
        "daily_up_trend_down": "这一次颠簸，没有改写路线",
        "daily_down_trend_up": "这一站回落，路线仍未改变",
        "stable": "列车在走，路线近似平稳",
    },
    "music": {
        "insufficient": "序曲已经落针",
        "daily_up_trend_down": "今天升调，整首歌仍沿原来的旋律",
        "daily_down_trend_up": "今天降调，整首歌仍沿原来的旋律",
        "stable": "每一拍都不同，主旋律近似稳定",
    },
    "letter": {
        "insufficient": "今天，我只想请你继续认识我",
        "daily_up_trend_down": "今天的数字变了，但我没有突然变成另一个故事",
        "daily_down_trend_up": "今天的数字回落，也不需要替整段时间下结论",
        "stable": "我一直在变化，只是方向暂时很安静",
    },
    "identity": {
        "insufficient": "你是刚刚启程的人",
        "daily_up_trend_down": "你在看一段路，而不是审判一天",
        "daily_down_trend_up": "你在保存变化，而不是追逐一个数字",
        "stable": "你正在收藏身体的节律",
    },
    "generative": {
        "insufficient": "第一颗记录点已经亮起",
        "daily_up_trend_down": "一颗星偏离了轨迹，星群方向未变",
        "daily_down_trend_up": "一颗星回落，星群方向未变",
        "stable": "点在移动，星群近似稳定",
    },
}
...
def _analysis_details(analysis: Mapping[str, object]) -> dict:
    """Derive privacy-safe relative facts for style-specific editorial choices."""
    daily = [item for item in (analysis.get("daily_records") or []) if isinstance(item, Mapping)]
    values = []
    dates = []
    for item in daily:
        try:
            value = float(item.get("weight"))
        except (TypeError, ValueError):
            continue
        ...
        values.append(value)
        try:
            dates.append(datetime.strptime(str(item.get("date"))[:10], "%Y-%m-%d").date())
        except (TypeError, ValueError):
            pass
    ...
    gaps = [max((second - first).days - 1, 0) for first, second in zip(dates, dates[1:])]
    ...
    return {
        "net_value": _signed(net_delta) + (" kg" if net_delta is not None else ""),
        "range_value": ("%.1f kg" % range_delta) if range_delta is not None else "—",
        "mean_swing": ("%.2f kg" % mean_swing) if mean_swing is not None else "—",
        "max_swing": ("%.1f kg" % max_swing) if max_swing is not None else "—",
...
def _management_details(analysis: Mapping[str, object]) -> dict:
    ...
    has_management = bool(management)
    return {
        ...
        "synthesis_headline": str(synthesis.get("headline") or "体重之外的同期记录还在积累"),
        "synthesis_paragraph": str(synthesis.get("paragraph") or "目前主要有体重记录；摄入、运动与睡眠数据不足，因此这张卡不把单一数字解释成原因。"),
...
def _style_analysis_text(style_id: str, view: Mapping[str, object]) -> str:
    ...
    weight_fact = "体重层面：%s。" % view["core_status"]
    ...
    facts = {
        "weight": weight_fact + coverage_fact + "%s；%s；%s。" % (view["intake_fact"], view["activity_fact"], view["sleep_fact"]),
        "intake": "%s。%s%s另外，%s；%s。" % (view["intake_fact"], weight_fact, coverage_fact, view["activity_fact"], view["sleep_fact"]),
        "activity": "%s。%s%s另外，%s；%s。" % (view["activity_fact"], weight_fact, coverage_fact, view["intake_fact"], view["sleep_fact"]),
        "sleep": "%s。%s%s另外，%s；%s。" % (view["sleep_fact"], weight_fact, coverage_fact, view["intake_fact"], view["activity_fact"]),
        "recording": coverage_fact + "%s；%s；%s。%s" % (view["intake_fact"], view["activity_fact"], view["sleep_fact"], weight_fact),
...
def _synthesis_block(style_id: str, view: Mapping[str, object]) -> str:
    ...
    return '<%s class="analysis-note analysis-%s" data-situation-pattern="%s"><small>%s</small><strong>%s</strong><p>%s</p><span>%s</span></%s>' % (
        tag, _safe(style_id), _safe(view["situation_pattern"]), _safe(ANALYSIS_LABELS[style_id]),
...
def _story_frame(style_id: str, state: str, view: Mapping[str, str]) -> dict:
    ...
    trend = view["trend_value"] if not insufficient else "还不足以判断"
    frames = {
        "weather-now": ("此刻天气", view["headline"], "今天的秤面是 %s；长期气流为 %s。" % (view["daily_value"], trend)),
        ...
        "ticket-journey": ("运动行程", "一张从 %s 个运动日出发的车票" % view["activity_days"], view["activity_fact"] + "；目的地不是某个体重数字。"),
        "passport-stamps": ("阶段入境章", "获得“%s”观察印章" % view["persona"], "印章只收藏观察行为，不奖励某个体重方向。"),
        "vinyl-record": ("运动唱片", "A 面 %s 个运动日，B 面总时长 %s" % (view["activity_days"], view["activity_value"]), view["activity_fact"] + "。"),
        "weekly-single": ("本周综合单曲", "《%s》" % view["synthesis_headline"], "体重、摄入、运动和睡眠共同写进 liner notes。"),
...
def _headline(family: str, state: str, variant: str) -> str:
    base, _ = CORE_STATE_COPY.get(state, CORE_STATE_COPY["insufficient"])
    value = FAMILY_HEADLINES.get(family, {}).get(state, base)
    if variant == "B" and family == "editorial":
        return "今日观察：" + base
    if variant == "B" and family == "letter" and state == "insufficient":
...
def _metrics(view: Mapping[str, str], variant: str = "default") -> str:
    profiles = {
        ...
        "journey": (
            ("已经过", view["span_days"] + " 天", "目的地不是某个体重"),
...
def _weather_visual(analysis: Mapping[str, object], view: Mapping[str, str], variant: str) -> str:
    ...
    if variant == "B":
        recorded = int(view.get("sleep_days") or 0)
        cells = "".join(
...
def _direction_visual(analysis: Mapping[str, object], view: Mapping[str, str], variant: str) -> str:
    angle = max(-32.0, min(32.0, float(analysis.get("trend_delta") or 0.0) * 18.0))
    if variant == "B":
        return """<div class="logbook"><div class="logbook-title"><small>CAPTAIN'S LOG / %s</small><strong>%s</strong></div>
          <dl><div><dt>运动记录日</dt><dd>%s 天</dd></div><div><dt>已记录总时长</dt><dd>%s</dd></div><div><dt>后半段变化</dt><dd>%s</dd></div></dl>
          <div class="logbook-chart">%s</div></div>%s%s""" % (
            view["edition"], view["story_headline"], view["activity_days"], view["activity_value"], view["activity_change"],
...
def _capsule_visual(analysis: Mapping[str, object], view: Mapping[str, str], variant: str) -> str:
    if variant == "B":
        return """<div class="future-letter"><div class="future-stamp">%s</div><small>TO MY FUTURE SELF</small>
          <p>%s</p><div class="future-note">%s<br>今天的秤面是 %s，长期趋势是 %s。</div></div>
          %s%s""" % (
            view["edition"], view["story_headline"], view["story_subhead"], view["daily_value"],
            view["trend_value"], _metrics(view, "sleep"), _moment(view)
        )
    return """<div class="capsule-stage"><div class="capsule-seal"><small>SEALED</small><strong>%s</strong><span>%s</span></div>
      ...
      %s%s""" % (view["intake_days"], "个饮食记录日", view["story_headline"], _metrics(view, "intake"), _moment(view))
    ...
    if variant == "B":
        ...
        return """<div class="moon-board"><div class="moon-title"><small>OBSERVATION PHASES</small><strong>%s</strong><p>%s</p></div>
          <div class="moon-grid" aria-label="由记录覆盖生成的抽象月相">%s</div></div>%s%s%s""" % (
            view["sleep_value"], view["story_subhead"], phases, _metrics(view, "sleep"), _moment(view), _context(view)
        )
...
def _journey_visual(analysis: Mapping[str, object], view: Mapping[str, str], variant: str) -> str:
    ...
    return """<div class="ticket"><div class="ticket-main"><small>MEDIWISE / PERSONAL LINE</small>
      ...
      %s%s""" % (view["story_headline"], view["edition"], view["activity_value"], _metrics(view, "activity"), _moment(view))
...
def _music_visual(analysis: Mapping[str, object], view: Mapping[str, str], variant: str) -> str:
    if variant == "B":
        return """<div class="single-stage"><div class="single-cover"><small>MW / SINGLE</small><div class="single-wave">%s</div><strong>%s</strong><span>%s</span></div>
          <div class="single-notes"><small>THIS WEEK'S LINER NOTES</small><p>%s</p><dl><div><dt>摄入</dt><dd>%s</dd></div><div><dt>运动</dt><dd>%s</dd></div><div><dt>睡眠</dt><dd>%s</dd></div></dl></div></div>%s""" % (
            _sparkline(analysis, "music-signal"), view["story_headline"], view["edition"], view["story_subhead"],
            view["intake_value"], view["activity_value"], view["sleep_value"], _moment(view)
        )
    return """<div class="record-stage"><div class="vinyl"><div class="vinyl-label"><small>MW</small><strong>%s</strong></div></div>
      <div class="tracklist"><small>NOW PLAYING</small><strong>%s</strong><ol><li>运动记录 <b>%s 天</b></li><li>已记录时长 <b>%s</b></li><li>后半段变化 <b>%s</b></li></ol></div></div>
...
def _view_model(
    ...
    core_title, core_status = CORE_STATE_COPY.get(state, CORE_STATE_COPY["insufficient"])
    ...
    if daily_delta is None:
        daily_note = "等待下一次记录"
    trend_note = "稳健估计" if analysis.get("trend_claim_allowed") else "记录不足，暂不判断"
    exact_weight = "绝对体重已隐藏"
    exact_weight_footer = ""
    if show_exact_weight and latest_weight is not None:
        exact_weight = "当前 %.1f kg" % float(latest_weight)
        exact_weight_footer = " · " + exact_weight
    date_label = "本次观察"
    ...
    view = {
        ...
        "state_conflict": state in ("daily_up_trend_down", "daily_down_trend_up"),
        "core_title": _safe(core_title),
        "core_status": _safe(core_status),
        "headline": _safe(_headline(family, state, style.get("variant", "A"))),
        "daily_label": _safe("秤面" + daily_note),
        "daily_value": _safe(_signed(daily_delta) + (" kg" if daily_delta is not None else "")),
        "daily_note": _safe(exact_weight),
        "trend_value": _safe(_signed(trend_delta) + (" kg" if trend_delta is not None else "")),
        "trend_note": _safe(trend_note),
        "coverage": _safe("%d / %d 天" % (int(analysis.get("recorded_days") or 0), int(analysis.get("window_days") or 0))),
        "confidence": _safe(str(analysis.get("confidence_label") or "不足")),
        "recorded_days": _safe(str(int(analysis.get("recorded_days") or 0))),
        "recorded_unit": "个记录日",
        "span_days": _safe(str(int(analysis.get("span_days") or 0))),
    ...
    management_details = _management_details(analysis)
...
def render_weight_story_html(
    ...
    *,
    ...
    view = _view_model(
        ...
        show_exact_date, context_lines,
    )
    content = renderer(analysis, view, variant) + _synthesis_block(style_id, view)
...
<body data-share-safe="%s" data-style-id="%s" data-content-role="%s" data-renderer="weight-story-v2"><div id="viewport" class="viewport"><main id="artboard" class="artboard family-%s variant-%s" data-layout-mode="%s" data-primary-domain="%s" data-palette="%d">
...
<footer><div><b>%s</b>%s</div><div>%s%s%s%s</div></footer>
        ...
        _safe(catalog_style.layout_mode), _safe(catalog_style.preferred_domains[0]), palette,
        ...
        content, _safe(privacy), _safe(DISCLAIMER), _safe(catalog_style.signature),
